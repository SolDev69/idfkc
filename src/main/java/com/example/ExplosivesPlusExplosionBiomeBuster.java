...
public class ExplosivesPlusExplosionBiomeBuster {
	...
	public ExplosivesPlusExplosionBiomeBuster(World world, Entity entity, double d, double d1, double d2, float f) {
		this.worldObj = world;
	...
	public void doExplosionA() {
		...
		int i = mod_ExplosivesPlus.BiomeBusterExplosiveRadius;
				...
				for(k1 = 0; k1 < i; ++k1) {
					if(k == 0 || k == i - 1 || i1 == 0 || i1 == i - 1 || k1 == 0 || k1 == i - 1) {
						...
						for(float d11 = 0.3F; i3 > 0.0F; i3 -= d11 * (12.0F / 16.0F)) {
							...
							if(d13 > 0) {
								i3 -= (Block.getBlockById(d13).getExplosionResistance(this.exploder) + 0.3F) * d11;
							}

							if(mod_ExplosivesPlus.EnhancedExplosions) {
		...
		List list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this.exploder, AxisAlignedBB.getBoundingBox((double)k, (double)k1, (double)i2, (double)i1, (double)var29, (double)var30));
		...
		for(int arraylist = 0; arraylist < list.size(); ++arraylist) {
			...
			if(chunkposition <= 1.0D) {
				...
				var33.attackEntityFrom(DamageSource.generic, (int)((var41 * var41 + var41) / 2.0D * 8.0D * (double)this.explosionSize + 1.0D));
		...
		if(this.isFlaming) {
			for(int var34 = var32.size() - 1; var34 >= 0; --var34) {
				...
				int var36 = var35.chunkPosX;
				int var37 = var35.chunkPosY;
				int k3 = var35.chunkPosZ;
				int var38 = this.worldObj.getBlockId(var36, var37, k3);
				int i4 = this.worldObj.getBlockId(var36, var37 - 1, k3);
				if(var38 == 0 && Block.getBlockById(i4).isOpaqueCube() && this.ExplosionRNG.nextInt(3) == 0) {
					this.worldObj.setBlock(var36, var37, k3, Blocks.fire);
				}
			}
		}
	...
	public void doExplosionB(boolean flag) {
		...
		for(int i = arraylist.size() - 1; i >= 0; --i) {
			...
			if(i1 > 0) {
				...
				Block.getBlockById(i1).onBlockDestroyedByExplosion(this.worldObj, j, k, l, null);//new Explosion(worldObj, exploder, explosionX, explosionY, explosionZ, explosionSize));
			}
		}
